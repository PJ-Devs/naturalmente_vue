...
const userSchema = yup.object().shape({
  name: yup
    ...
    .max(255, 'El nombre no puede tener mas de 255 caracteres')
    .matches(/^[a-zA-Z\s-]+$/, 'El nombre solo puede contener letras, espacios y guiones'),
  email: yup
    .string()
    .required('El email es requerido')
    .min(7, 'El email no puede tener menos de 7 caracteres')
    .max(255, 'El email no puede tener mas de 255 caracteres')
    .email('El email debe ser valido'),
  phone_number: yup
    .string()
    .min(4, 'El numero de telefono no puede tener menos de 4 caracteres')
    .max(30, 'El numero de telefono no puede tener mas de 30 caracteres')
    .matches(/^[a-zA-Z0-9]+$/, 'El numero de telefono solo puede contener letras y numeros'),
  address: yup
    .string()
    .min(8, 'La direccion no puede tener menos de 8 caracteres')
    .max(100, 'La direccion no puede tener mas de 100 caracteres')
    .matches(
      /^.*$/u,
      'La direccion solo puede contener letras, numeros, espacios, guiones y almohadillas'
    )
})
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mx-auto">
          <template v-for="(input, index) in inputs" :key="index">
            <GenericInput
              ...
              :value="useAuthUser?.getUser[input.name]"
              :label="input.label"
              @input="updateUserField(input.name, $event.target.value)"
            />
          </template>
        </div>
